...
import Card from "@/components/atom/CardMobile";
...
export default function HomeMobile() {
  ...
  useEffect(() => {
    ...
    const initial = saved || "light"; // 👈 paksa default ke light
    setTheme(initial);
  ...
  return (
    <div className="w-screen h-dvh bg-white-primary flex justify-center items-center select-none overflow-hidden relative transition-colors duration-300">
      ...
      {/* 🌞/🌙 Theme Toggle — posisi tetap */}
        <img
          ...
          className={`absolute w-[50px] top-5 left-5 z-20 cursor-pointer transition-transform duration-300 ${
            isScaling ? "scale-75" : "scale-100"
          }`}
        />

      {/* 💼 Card utama */}
      <div className="relative w-[750px] h-[500px] top-10 p-4 z-10 transition-colors duration-300">
        ...
        <div className="flex flex-col items-center relative z-10">
          <Text />
          <div className="mt-5 grid grid-cols-2 grid-rows-2 justify-center gap-12 gap-x-16 w-fit h-fit p-5">
      ...
      <Motion className="bottom-0" isVisible={showAbout}>
        <About onClose={() => setShowAbout(false)} />
      </Motion>
      <Motion className="bottom-0" isVisible={showProject}>
